...
export const hasPerm = {
    mounted(el, binding) {
        ...
        if (!requiredPerms || (typeof requiredPerms !== "string" && !Array.isArray(requiredPerms))) {
            throw new Error("需要提供权限标识！例如：v-hasPerm=\"'sys:user:create'\" 或 v-hasPerm=\"['sys:user:create', 'sys:user:update']\"");
        }
...
export const hasRole = {
    mounted(el, binding) {
        ...
        if (!requiredRoles || (typeof requiredRoles !== "string" && !Array.isArray(requiredRoles))) {
            throw new Error("需要提供角色标识！例如：v-hasRole=\"'ADMIN'\" 或 v-hasRole=\"['ADMIN', 'TEST']\"");
        }
