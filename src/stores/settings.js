...
import { applyTheme, generateThemeColors, resolveThemeMode, toggleDarkMode, toggleSidebarColor, watchSystemTheme, } from "@/utils/theme";
...
export const useSettingsStore = defineStore("setting", () => {
    ...
    const pageSwitchingAnimation = useStorage(STORAGE_KEYS.PAGE_SWITCHING_ANIMATION, defaults.pageSwitchingAnimation);
    // 布局
    const layout = useStorage(STORAGE_KEYS.LAYOUT, defaults.layout);
    const sidebarColorScheme = useStorage(STORAGE_KEYS.SIDEBAR_COLOR_SCHEME, defaults.sidebarColorScheme);
    // 主题
    ...
    function setGlassParam(key, value) {
    ...
    watch([resolvedTheme, themeColor], ([t, c]) => {
        toggleDarkMode(t === "dark" /* ThemeMode.DARK */);
        applyTheme(generateThemeColors(c, t));
    }, { immediate: true });
    watch(sidebarColorScheme, (v) => toggleSidebarColor(v === "classic-blue" /* SidebarColor.CLASSIC_BLUE */), {
        immediate: true,
    });
    // 玻璃效果
    watch(glassEffect, (v) => {
