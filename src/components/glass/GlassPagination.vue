<template>
  ...
  >
    ...
    <template v-for="(p, i) in pages" :key="i">
      <span v-if="p === 'ellipsis'" class="px-1 text-muted-foreground text-sm select-none">
        &hellip;
      </span>
      <button
        v-else
...
const btnBase =
  "inline-flex items-center justify-center h-8 min-w-8 px-1.5 text-sm rounded-md transition-colors cursor-pointer select-none";
