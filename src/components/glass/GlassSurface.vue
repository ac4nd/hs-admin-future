...
const containerStyle = computed(() => {
  ...
  return {
    ...
    background: config.value.layer === 'liq-clear'
      ? (ol ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.02)')
      : (ol ? 'rgba(255,255,255,0.12)' : 'rgba(30,30,35,0.3)'),
    ...
    ...(shouldTrackMouse.value && effectiveElasticity.value
      ? { transform: elasticTransform.value, transition: 'transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94)' }
      : {}),
  }
})
</script>
...
<template>
  <component
    :is="as"
    ref="surfaceRef"
    :class="cn('glass-surface relative overflow-hidden', shouldTrackMouse && 'glass-surface--elastic', props.class)"
  ...
  >
    ...
    <!-- 用户内容 -->
    <div class="relative z-[1] flex w-full h-full" :style="{ alignItems: 'inherit', justifyContent: 'inherit', gap: 'inherit', flexDirection: 'inherit' }">
      <slot />
    </div>
