    ...
    <SheetContent class="sm:max-w-5xl overflow-auto">
      ...
      <div v-else-if="currentStep === 0" class="space-y-4">
        ...
        <Card>
          <CardContent class="pt-5 space-y-3">
            <h3 class="text-sm font-semibold text-muted-foreground">{{ t('codegen.tableInfo') }}</h3>
            <div class="grid grid-cols-2 gap-3">
              ...
              <div class="space-y-1.5">
                <Label>{{ t('codegen.businessName') }} <span class="text-destructive">*</span></Label>
                <Input v-model="config.businessName!" :placeholder="t('codegen.businessNamePlaceholder')" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent class="pt-5 space-y-3">
            <h3 class="text-sm font-semibold text-muted-foreground">{{ t('codegen.packageInfo') }}</h3>
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1.5">
                <Label>{{ t('codegen.packageName') }}</Label>
                <Input v-model="config.packageName!" :placeholder="t('codegen.packageNamePlaceholder')" />
              </div>
              <div class="space-y-1.5">
                <Label>{{ t('codegen.moduleName') }}</Label>
                <Input v-model="config.moduleName!" :placeholder="t('codegen.moduleNamePlaceholder')" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent class="pt-5 space-y-3">
            <h3 class="text-sm font-semibold text-muted-foreground">{{ t('codegen.genConfig') }}</h3>
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1.5">
                <Label>{{ t('codegen.entityName') }} <span class="text-destructive">*</span></Label>
                <Input v-model="config.entityName!" :placeholder="t('codegen.entityNamePlaceholder')" />
              </div>
              <div class="space-y-1.5">
              ...
              <div class="space-y-1.5">
                ...
                <Input v-model="config.removeTablePrefix!" :placeholder="t('codegen.removePrefixPlaceholder')" />
              </div>
              <div class="space-y-1.5">
                <Label>{{ t('codegen.pageType') }}</Label>
                <div class="flex items-center gap-3 pt-1">
                  <label class="flex items-center gap-1.5 cursor-pointer text-sm">
                    <input type="radio" value="classic" v-model="config.pageType" class="accent-primary" />
                    {{ t('codegen.pageTypeClassic') }}
                  </label>
                  <label class="flex items-center gap-1.5 cursor-pointer text-sm">
                    <input type="radio" value="curd" v-model="config.pageType" class="accent-primary" />
            ...
            <div class="space-y-1.5">
              ...
              <Select v-model="config.parentMenuId!">
                ...
                <SelectContent>
                  <SelectItem v-for="opt in menuOptions" :key="String(opt.value)" :value="String(opt.value)">
                    {{ opt.label }}
                  </SelectItem>
                </SelectContent>
              </Select>
      ...
      <div v-else-if="currentStep === 1" class="space-y-3">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3 text-sm">
            ...
            <Badge variant="outline" class="text-green-600">{{ t('codegen.fieldQuery') }}: {{ queryCount }}</Badge>
            <Badge variant="outline" class="text-yellow-600">{{ t('codegen.fieldList') }}: {{ listCount }}</Badge>
            <Badge variant="outline" class="text-blue-600">{{ t('codegen.fieldForm') }}: {{ formCount }}</Badge>
          </div>
          <div class="flex items-center gap-1">
            <Button variant="outline" size="sm" @click="batchSet('query', 1)">{{ t('codegen.queryLabel') }}{{ t('codegen.selectAll') }}</Button>
            <Button variant="outline" size="sm" @click="batchSet('query', 0)">{{ t('codegen.queryLabel') }}{{ t('codegen.deselectAll') }}</Button>
            <Button variant="outline" size="sm" @click="batchSet('list', 1)">{{ t('codegen.listLabel') }}{{ t('codegen.selectAll') }}</Button>
            <Button variant="outline" size="sm" @click="batchSet('list', 0)">{{ t('codegen.listLabel') }}{{ t('codegen.deselectAll') }}</Button>
            <Button variant="outline" size="sm" @click="batchSet('form', 1)">{{ t('codegen.formLabel') }}{{ t('codegen.selectAll') }}</Button>
            <Button variant="outline" size="sm" @click="batchSet('form', 0)">{{ t('codegen.formLabel') }}{{ t('codegen.deselectAll') }}</Button>
          </div>
        </div>
            ...
            <TableBody>
              ...
              <TableRow v-for="(field, idx) in fields" :key="idx" class="hover:bg-muted/50">
                ...
                <TableCell class="text-center">
                  <input type="checkbox" v-model="field.isShowInQuery" :true-value="1" :false-value="0" class="size-4 rounded accent-primary" />
                </TableCell>
                <TableCell>
                  <Select :model-value="String(field.queryType)" @update:model-value="(v: any) => field.queryType = Number(v)">
                    <SelectTrigger class="h-7 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem v-for="qt in queryTypeOptions" :key="String(qt.value)" :value="String(qt.value)">
                        {{ qt.label }}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <TableCell class="text-center">
                  <input type="checkbox" v-model="field.isShowInList" :true-value="1" :false-value="0" class="size-4 rounded accent-primary" />
                </TableCell>
                <TableCell class="text-center">
                  <input type="checkbox" v-model="field.isShowInForm" :true-value="1" :false-value="0" class="size-4 rounded accent-primary" />
                </TableCell>
                <TableCell>
                  <Select :model-value="String(field.formType)" @update:model-value="(v: any) => field.formType = Number(v)">
                    <SelectTrigger class="h-7 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem v-for="ft in formTypeOptions" :key="String(ft.value)" :value="String(ft.value)">
                        {{ ft.label }}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                ...
                <TableCell class="text-center">
                  <input type="checkbox" v-model="field.isRequired" :true-value="1" :false-value="0" class="size-4 rounded accent-primary" />
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
      ...
      <div v-else-if="currentStep === 2" class="space-y-3">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <label v-for="s in scopeOptions" :key="s.value" class="flex items-center gap-1.5 cursor-pointer text-sm">
              <input type="radio" :value="s.value" v-model="previewScope" class="accent-primary" />
              {{ s.label }}
            </label>
          </div>
          <div class="flex items-center gap-2">
        ...
        <div v-else class="flex border rounded-md overflow-hidden" style="height: 500px">
          ...
          <div class="w-64 border-r overflow-auto bg-muted/30 shrink-0">
            <div class="p-2 text-xs font-semibold text-muted-foreground border-b">
              {{ t('codegen.fileTree') }} ({{ filteredPreviewFiles.length }} {{ t('codegen.files') }})
            </div>
            <div class="p-1">
              <button
                ...
                :class="{ 'bg-primary/10 text-primary font-medium': selectedFile?.path === file.path }"
                @click="selectedFile = file"
              >
                <Badge variant="outline" class="text-[10px] px-1 py-0 h-4" :class="file.scope === 'frontend' ? 'text-green-600' : 'text-orange-500'">
          ...
          <div class="flex-1 overflow-auto">
            <div v-if="selectedFile" class="h-full flex flex-col">
              <div class="px-3 py-1.5 border-b bg-muted/50 flex items-center gap-2 shrink-0">
                <span class="text-xs font-mono text-muted-foreground truncate">{{ selectedFile.path }}</span>
                <Badge variant="outline" class="text-[10px] px-1 py-0 h-4 shrink-0">{{ selectedFile.language.toUpperCase() }}</Badge>
              </div>
              <pre class="flex-1 p-3 text-xs font-mono overflow-auto whitespace-pre-wrap"><code>{{ selectedFile.content }}</code></pre>
            </div>
      ...
      <SheetFooter class="mt-4">
        <div class="flex items-center justify-between w-full">
          ...
          <span v-else />
          <div class="flex items-center gap-2">
            <Button variant="outline" @click="emit('update:visible', false)">{{ t('codegen.cancel') }}</Button>
...
import {
  ChevronRightIcon, SettingsIcon, LayoutGridIcon, EyeIcon,
  CopyIcon, DownloadIcon, Loader2Icon,
} from "@lucide/vue";
...
import { getGenConfig, saveGenConfig, getPreviewData, getMenuOptions } from "@/api/codegen";
...
function loadConfig(tableName: string) {
  const data = getGenConfig(tableName);
...
function loadPreview(tableName: string) {
  previewFiles.value = getPreviewData(tableName);
...
function handleDownloadZip() {
  // Mock: 简单下载当前选中文件内容为文本
  toast.info("ZIP 下载需接入后端 API");
}
...
function handleNext() {
  if (currentStep.value === 0) {
    // 验证基础配置
    if (!config.businessName) { toast.error(t("codegen.businessNameRequired")); return; }
    if (!config.entityName) { toast.error(t("codegen.entityNameRequired")); return; }
    currentStep.value = 1;
  } else if (currentStep.value === 1) {
    // 保存配置 → 生成预览
    generating.value = true;
    saveGenConfig(props.tableName, { ...config });
    setTimeout(() => {
      loadPreview(props.tableName);
      currentStep.value = 2;
