<template>
  <div>
    <button
      ...
      :class="{ 'bg-primary/10 text-primary font-medium': String(node.value) === selectedId }"
      @click="handleToggle"
    >
      <span
    ...
    <div v-if="expanded && node.children?.length" class="ml-3">
      <DeptTreeNode
        ...
        @select="(v: string | number | undefined) => emit('select', v)"
      />
    </div>
  </div>
</template>
...
function hasMatch(node: OptionItem, kw: string): boolean {
  if (node.label.includes(kw)) return true;
