      ...
      <CardContent class="space-y-4">
            ...
            <PopoverTrigger as-child>
              <Button variant="outline" class="w-80 justify-start font-normal">
                <span v-if="selectedUser" class="flex items-center gap-2">
                  <Badge variant="secondary" class="text-[10px]">{{ selectedUser.status === 1 ? "启用" : "禁用" }}</Badge>
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge :variant="user.status === 1 ? 'default' : 'secondary'" class="text-[10px]">
                          {{ user.status === 1 ? "启用" : "禁用" }}
                        </Badge>
                      </TableCell>
                    </TableRow>
        ...
        <div v-if="selectedUser" class="mt-4 p-4 border rounded-lg bg-muted/30">
          ...
          <div class="grid grid-cols-2 gap-2 text-sm">
            <div><span class="text-muted-foreground">用户名：</span>{{ selectedUser.username }}</div>
            <div><span class="text-muted-foreground">昵称：</span>{{ selectedUser.nickname }}</div>
            <div><span class="text-muted-foreground">部门：</span>{{ selectedUser.deptName }}</div>
            <div><span class="text-muted-foreground">角色：</span>{{ selectedUser.roleNames }}</div>
          </div>
        </div>
      </CardContent>
    </Card>
...
const mockUsers: MockUser[] = [
  { id: "1", username: "admin", nickname: "管理员", deptName: "研发部门", roleNames: "超级管理员", status: 1 },
  { id: "2", username: "zhangsan", nickname: "张三", deptName: "市场部门", roleNames: "普通用户", status: 1 },
  { id: "3", username: "lisi", nickname: "李四", deptName: "财务部门", roleNames: "审计员", status: 1 },
  { id: "4", username: "wangwu", nickname: "王五", deptName: "人事部门", roleNames: "HR", status: 0 },
  { id: "5", username: "zhaoliu", nickname: "赵六", deptName: "研发部门", roleNames: "开发工程师", status: 1 },
  { id: "6", username: "sunqi", nickname: "孙七", deptName: "测试部门", roleNames: "测试工程师", status: 1 },
];
