      ...
      <CardContent class="pt-5 pb-4">
        <div class="flex flex-wrap items-end gap-3">
          <div class="space-y-1.5">
            ...
            <Input v-model="queryParams.keywords" placeholder="用户名/邮箱" class="w-52 h-8 text-sm" @keyup.enter="handleQuery" />
          </div>
          <div class="space-y-1.5">
      ...
      <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead class="w-20">
                  <Button variant="ghost" size="sm" class="h-6 px-1 text-xs" @click="toggleSort('status')">
                    状态 {{ sortField === 'status' ? (sortOrder === 'asc' ? '↑' : '↓') : '' }}
                  </Button>
                </TableHead>
                <TableHead class="w-36">
                  <Button variant="ghost" size="sm" class="h-6 px-1 text-xs" @click="toggleSort('createTime')">
            ...
            <TableBody>
              <template v-if="filteredList.length === 0">
                <TableRow>
                  <TableCell :colspan="10" class="h-20 text-center text-muted-foreground">暂无数据</TableCell>
                </TableRow>
              </template>
                ...
                <TableRow :data-state="checkedIds.has(row.id) ? 'selected' : undefined">
                  ...
                  <TableCell>
                    <button class="p-0.5 hover:bg-muted rounded" @click="toggleExpand(row.id)">
                      <ChevronRightIcon class="size-3.5 transition-transform" :class="{ 'rotate-90': expandedIds.has(row.id) }" />
                    </button>
                  </TableCell>
                ...
                <TableRow v-if="expandedIds.has(row.id)">
                  <TableCell :colspan="10" class="bg-muted/30 px-8 py-3">
                    <div class="grid grid-cols-3 gap-4 text-sm">
                      <div><span class="text-muted-foreground">ID：</span>{{ row.id }}</div>
                      <div><span class="text-muted-foreground">用户名：</span>{{ row.username }}</div>
                      <div><span class="text-muted-foreground">角色：</span>{{ row.roles }}</div>
                      <div><span class="text-muted-foreground">手机号：</span>{{ row.phone }}</div>
                      <div><span class="text-muted-foreground">邮箱：</span>{{ row.email }}</div>
                      <div><span class="text-muted-foreground">创建时间：</span>{{ row.createTime }}</div>
                    </div>
                  </TableCell>
                </TableRow>
              </template>
        ...
        <div class="flex items-center justify-between mt-4">
          <div class="text-xs text-muted-foreground">
            共 {{ filteredList.length }} 条，启用 {{ filteredList.filter(r => r.status).length }} 条
          </div>
          <div class="flex items-center gap-2">
    ...
    <Dialog :open="dialogOpen" @update:open="dialogOpen = $event">
      <DialogContent class="sm:max-w-md">
        ...
        <div class="space-y-4 py-2">
          ...
          <div class="flex items-center justify-between">
            <Label>状态</Label>
            <Switch :checked="formData.status" @update:checked="(v: boolean) => formData.status = v" />
          </div>
        </div>
        <DialogFooter>
...
const mockData: Row[] = [
  { id: 1, username: "Richard Clark", roles: "editor", phone: "18185826431", email: "y.djf@xiswx.fk", status: true, createTime: "2010-04-17 12:39:20" },
  { id: 2, username: "Robert Garcia", roles: "admin", phone: "18125716043", email: "z.japgndxosu@inoudjxc.ie", status: false, createTime: "2020-01-02 11:51:58" },
  { id: 3, username: "Thomas Moore", roles: "admin", phone: "18106622048", email: "j.fvsgnjjutm@fmjw.se", status: true, createTime: "1983-10-12 10:06:41" },
  { id: 4, username: "Dorothy Lewis", roles: "admin", phone: "13321357284", email: "o.htso@iwxvehrs.tj", status: true, createTime: "1970-03-03 00:26:45" },
  { id: 5, username: "George Rodriguez", roles: "admin", phone: "18158641167", email: "x.sigizx@fwknokiqn.tr", status: true, createTime: "1988-03-16 14:46:26" },
  { id: 6, username: "Angela Jackson", roles: "admin", phone: "19810721230", email: "j.gqrdqaqtu@ipthgm.fj", status: true, createTime: "2006-09-26 12:53:37" },
  { id: 7, username: "James Walker", roles: "admin", phone: "18123903251", email: "k.axmdcsl@mcmeudog.cl", status: true, createTime: "1981-01-19 12:51:34" },
  { id: 8, username: "Paul Garcia", roles: "admin", phone: "18617930381", email: "c.glufsn@vwqntlllj.es", status: false, createTime: "2009-12-04 20:40:57" },
  { id: 9, username: "Jeffrey Miller", roles: "admin", phone: "18145245413", email: "u.poqrqw@arto.rw", status: false, createTime: "1991-04-01 05:16:52" },
  { id: 10, username: "Donna Lewis", roles: "editor", phone: "19839835537", email: "l.lmpeoupu@rujdlzdbk.gf", status: true, createTime: "1987-11-29 21:47:37" },
  { id: 11, username: "Jennifer Smith", roles: "editor", phone: "18145245413", email: "j.jqx@xjxqx.jp", status: true, createTime: "1991-04-01 05:16:52" },
];
...
const filteredList = computed(() => {
  ...
  if (sortField.value) {
    list.sort((a, b) => {
      const aVal = sortField.value === "status" ? (a.status ? 1 : 0) : a[sortField.value as keyof Row] as string;
      const bVal = sortField.value === "status" ? (b.status ? 1 : 0) : b[sortField.value as keyof Row] as string;
...
const formData = reactive<{ id?: number; username: string; roles: string; status: boolean }>({
  username: "", roles: "user", status: true,
});
...
function openEdit(row: Row) {
  isEdit.value = true;
  Object.assign(formData, { id: row.id, username: row.username, roles: row.roles, status: row.status });
  dialogOpen.value = true;
}

function handleSubmit() {
  if (!formData.username) { toast.error("请输入用户名"); return; }
  if (isEdit.value && formData.id) {
    const idx = dataList.value.findIndex((r) => r.id === formData.id);
    if (idx >= 0) dataList.value[idx] = { ...dataList.value[idx], username: formData.username, roles: formData.roles, status: formData.status };
    toast.success("修改成功");
  } else {
    dataList.value.push({
      id: Date.now(),
