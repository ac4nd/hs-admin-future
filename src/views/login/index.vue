...
<template>
  ...
  >
    ...
    <template v-if="isDark">
      <div class="absolute inset-0 bg-cover bg-center bg-no-repeat" :style="{ backgroundImage: `url(${bgDarkUrl})` }" />
      ...
      <div class="toolbar-pill flex items-center gap-1">
        ...
        <div class="relative">
          <button
            ...
            @click.stop="showThemeMenu = !showThemeMenu; showLangMenu = false"
          >
            <!-- Sun -->
            <svg v-if="themeIcon === 'sun'" class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
            <!-- Moon -->
            <svg v-else-if="themeIcon === 'moon'" class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Transition name="dropdown">
            <div v-if="showThemeMenu" class="dropdown-menu" @click.stop>
              ...
              >
                {{ opt.label.value }}
                <svg v-if="settingsStore.theme === opt.value" class="w-3.5 h-3.5 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="relative">
          <button
            ...
            @click.stop="showLangMenu = !showLangMenu; showThemeMenu = false"
          >
          ...
          <Transition name="dropdown">
            <div v-if="showLangMenu" class="dropdown-menu" @click.stop>
              ...
              >
                {{ opt.label }}
                <svg v-if="appStore.language === opt.value" class="w-3.5 h-3.5 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
                </svg>
              </button>
            </div>
    ...
    <!-- 主内容区: 两栏布局 -->
    <div class="relative z-10 flex-1 grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-6 md:gap-8 items-center px-4 sm:px-6 md:px-12 lg:px-16 max-w-[1280px] xl:max-w-[1400px] mx-auto w-full">
      <!-- 左侧 Hero -->
        ...
        <div class="w-full p-8 space-y-6">
          ...
          <form class="space-y-5" @submit.prevent="handleLogin">
            ...
            <div class="space-y-1.5">
              ...
              <GlassSurface preset="input" class="glass-field w-full">
                <div class="w-full flex items-center px-3 py-2.5 gap-2 relative z-[1]">
                  <svg class="w-4 h-4 shrink-0 login-text-sub" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <GlassSurface preset="input" class="glass-field w-full">
                <div class="w-full flex items-center px-3 py-2.5 gap-2 relative z-[1]">
                  ...
                  <input
                    ...
                    class="login-input flex-1 min-w-0 text-sm outline-none"
                  />
                  <button type="button" @click="showPassword = !showPassword" class="shrink-0 p-0.5 opacity-60 hover:opacity-100 transition-opacity">
                    <svg v-if="!showPassword" class="w-4 h-4 login-text-sub" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                    </svg>
                    <svg v-else class="w-4 h-4 login-text-sub" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-1.5">
              ...
              <div class="flex gap-2">
                ...
                >
                  <img v-if="captchaBase64" :src="captchaBase64" alt="captcha" class="h-full w-full object-contain" />
                  <div v-else class="h-full w-full flex items-center justify-center text-xs opacity-40 login-text">
                    ...
                  </div>
                </div>
              </div>
            ...
            <div class="flex items-center justify-between">
              <label class="flex items-center gap-2 cursor-pointer">
                <input v-model="rememberMe" type="checkbox" class="w-3.5 h-3.5 accent-blue-400 rounded" />
            ...
            <!-- 登录按钮 -->
            <GlassSurface preset="button" :mouse-tracking="true" :on-click="handleLogin" class="w-full">
          ...
          <div class="flex items-center justify-center gap-1.5 text-sm">
            ...
            <button type="button" class="font-medium hover:underline login-text">{{ t('login.register') }}</button>
          </div>

          <!-- 其他登录方式分割线 -->
          <div class="flex items-center gap-3">
            <div class="flex-1 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent" />
            <span class="text-xs opacity-40 whitespace-nowrap login-text">{{ t('login.otherLoginMethods') }}</span>
            <div class="flex-1 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent" />
          </div>
...
.login-gradient {
  position: absolute;
  inset: 0;
  background: linear-gradient(180deg, #005ec0 0%, #2a8ade 30%, #7ec8e3 55%, #f5d76e 80%, #fee070 100%);
